...
try {
    ...
    & docker run --rm -v "${repoRoot}/prometheus:/etc/prometheus:ro" --entrypoint promtool prom/prometheus:v3.12.0 check config /etc/prometheus/prometheus.yml
    if ($LASTEXITCODE -ne 0) {
    ...
    & docker run --rm -v "${repoRoot}:/workspace:ro" --entrypoint promtool prom/prometheus:v3.12.0 check rules "/workspace/$dashboardRulesFileName"
    if ($LASTEXITCODE -ne 0) {
    ...
    if (-not $SkipGradleTest) {
        ...
        & docker run --rm -v "${demoServicePath}:/workspace" -w /workspace gradle:9.5.1-jdk21 gradle --no-daemon test
        if ($LASTEXITCODE -ne 0) {
